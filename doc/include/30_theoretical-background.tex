...
\todo{It is not quite clear that fields are methods as well, because above
fields *and* methods are mentioned}
...
\end{description}
\todo{This section seems to have errors}
...
leads us to the first conflict: sharing types vs implementation. To
...
\subsection{Schizophrenic Self-Reference} \todo{Cite}
...
\begin{figure}
	\centering
	\subfigure[Naive recursion]{
		\includedot[scale=0.7]{dot/naiveRecursion}
	}
	\subfigure[mutual recursion]{
...
An example of schizophrenic self-reference for the second case could be
...
not support subsumption \todo{This word seems to occur for the first time here. Explain}
...
%receiver of the method, i.e., the \self reference in the method. The
...
%	}
%	
%	// Not typeable expression in Java due to the contravariance rule
%	public setNext(DoublyLinkedNode next) {
%			this.next = next;
%			next.prev = this;
%	}
%
%	// Covariance at return type

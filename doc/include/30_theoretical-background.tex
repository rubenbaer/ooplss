...
\autoref{lst:javaBinary} shows how subtyping can not prevent the
possibility to add objects of type \emph{SinglyLinkedNode} in a
\emph{DoublyLinkedNode} which is definitely incorrect and has to end in a
runtime exception. In subtyping existing object state variables can not
be further specialised and the method \emph{setNext} can not specialise
the argument type. The \autoref{lst:javaBinary1} and \ref{lst:javaBinary2}
shows how the type system has to be escaped by casts which is unsafe and.
However, these problems does not rise from binary methods them self but
in combination with subtyping and inheritance since these concepts are
associated with subsumption
...
public class DoublyLinkedNode extends SinglyLinkedNode {
  ...
  public setNext(SinglyLinkedNode next) {
    if (next instanceof DoublyLinkedNode) {
      %\label{lst:javaBinary1})DoublyLinkedNode dNext = (DoublyLinkedNode)next;
      this.next = dNext;
      dNext.prev = this;
    } else {
      throw new Exception();
  ...
  public DoublyLinkedNode getNext() {
    %\label{lst:javaBinary2})return (DoublyLinkedNode)this.next;
  }
}
\end{lstlisting}
...
\emph{DoublyLinkedNode}. As seen in \autoref{cpt:subtypingVsSubclassing}
...
\section{Protocol}
\begin{lstlisting}[label={fig:minmax-example},caption={Max and MinMax protocol}]
class Max {
  n:Number;

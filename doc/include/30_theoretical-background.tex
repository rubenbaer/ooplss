...
\subsection{Variance}
Variance classifies the type change within the type hierarchy.
...
inheritance automatically creates a subtype. Of a practical point
...
\autoref{fig:implementationConflict} \cite{simons_theory_2003-4}:
% reference to simons?
...
has to use \emph{instanceof} to check whether the given object is of the same.
...
\autoref{sec:schizoReferences}. \\

In subclassing however, the question rises: how should return values
and arguments be typed (see case 2 in \autoref{sec:schizoReferences})
...
and $A$ \match $B$ does not imply $a:B$ how subtyping does.

...
use of the \mytype. 
\todo{This sentence does not work}
However, since a function in the lambda calculus
can not be recursive per se a special construct is needed. With
the use of a fixed point\nomenclature{Fix point}{A fix point of a
function $f$, regardless of whether the function is a type operator or
a function over $\mathbb{N}$, is a point where $f(x) = x$. Fix points
are important for recursive type functions} operator it is possible
...
theorem the $\mu$-convection \todo{convention?}
...
\todo{Should the \emph{type} be plural here? *one* type cannot diverge imho..}
To tackle
...
isomorphism\nomenclature{Isomorphism}{An isomorphism is a bijective

...
subtyping. \autoref{sec:sharingTypes} gives a deeper insight of this
problem.
...
understand this have a look at \autoref{fig:implementationConflict}
...
This solves the problem described in \autoref{sec:sharingTypes}. Since
subclassing does not create subtypes, the restriction of contravariance,
described in \autoref{sec:recursiveClosure}, is of no relevancy
...
structured?} in \autoref{sec:schizoReferences}. \\

In subclassing however, the question rises: how should return values and
arguments be typed (see the second case in \autoref{sec:schizoReferences})
...
\autoref{lst:javaBinary} contains some problems that can not be tackled
with subtyping. First of all the variable in \autoref{lst:javaBinary0} can
...
method on \autoref{lst:javaBinary1}.

The second problem lies in \autoref{lst:javaBinary2}. Since there is a covariant 
...
\autoref{lst:break} illustrates what behaviour would be possible if subtyping 
...
can not help in situations illustrated in \autoref{lst:matchbreak}.
...
\autoref{lst:generics} shows how this concept is implemented in a popular

...
\begin{listing}
	\begin{tabular}[H]{lrll}
		...
		CONSTRUCTOR & \lra & \\
%
%		\emph{Program} & PROG & \lra & CLASS+ \\
%		\emph{Classes} & CLASSES & \lra & CLASS+ \\
%		\emph{Class} & CLASS & \lra & `class' C$\langle$`['CLASSPPRMS`]'$\rangle$? COMBINATORS \\
%		& & & `\{' VAR* METHOD* CONSTRUCTOR* `\}' \\
%		\emph{Class parameters} & CLASSPRMS & \lra & CLASSPARAM $\langle$, CLASSPARAM $\rangle$* \\
%		\emph{Class parameter} & CLASSPARAM & \lra & TYPE \\
%		& & | & TYPEVAR \match TYPE \\
%		\emph{Class combinators} & COMBINATORS & \lra & SUBTYPEOF? SUBCLASSOF? \\
%		\emph{Subtype combinator} & SUBTYPEOF & \lra & `subtypeOf' TYPE \\
%		\emph{Subclass combinator} & SUBCLASSOF & \lra & `subclassOf' TYPE \\
%		\emph{Type} & TYPE & \lra & \\
%		\emph{Variable} & VAR & \lra & \\
%		\emph{Method} & METHOD & \lra & \\
%		\emph{Constrcutor} & CONSTRUCTOR & \lra & \\
%


%		\emph{Class}					& C	& $\lra $	& A[T] E[V] subtypeOf F[W], subclassOf D[U] {, D'[U']}* \{T a; T M;\} \\
%		\emph{Types}					& T & $\lra $	& A \\
%													&   & 		 | & A \match B[A] \\
%		\emph{Method}					& M	& $\lra $	& T m[S](U a, V b) \{e;\} \\
%		\emph{Constructor}		& K	& $\lra $	& c(U a, V b) \{A(a); e;\} \\
%		\emph{Expression}			& e & $\lra $	& x; \\
%													&   & 		 | & x = y; \\
%													&   & 		 | & e.f; \\
%													&   & 		 | & e.m(p); \\
%													&   & 		 | & new C(p); \\
...
the expression in listing \ref{lst:breakThis} can not be typed.

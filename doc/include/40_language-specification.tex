...
provides mutual recursion as wanted? Let's consider a method call of $m$
in \C. The method call handle goes correctly to its superclass method
$B.m$. Now there the next problem may occur. Since the base reference
...
proposition \ref{prop:matching}. Bruce is stated that `matching is the
...
\begin{lstlisting}[float=ht,language=ooplss,caption=Parametrisation in \ooplss,label=lst:parametrisationOoplss]
...
\begin{lstlisting}[float=ht,language=ooplss,caption=Valid use of a match-bound container,label=lst:useParametrisationOoplss]
class Main {
	def run() : Void {
...
\begin{lstlisting}[float=ht,language=java,caption=Container class translated to Java,label=lst:parametrisationJava]
...
\begin{lstlisting}[float=ht,language=java,caption=Use of parametised classes in Java,label=lst:useParametrisationJava]
public class Main {
	public void run() {
...
\begin{lstlisting}[float=ht,language=java,caption=Extending a container class translated to Java,label=lst:extendingContainer]
...
\begin{lstlisting}[float=ht,language=ooplss,caption=Explicit \mytype parametrisation,label=lst:explicitMyType]
class A[MyType <# A] {
	def m(o : MyType) MyType {

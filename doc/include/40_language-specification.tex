Inference \chapter{Language Specification}
\label{ctr:languageSpecification}
This chapter contains the language specification part of \ooplss.
In the first section defines the fundamental design principles used in
further discussion of the language. Afterwards a overview of the syntax
is given. It is just a abstract definition and differs in some points
from the real syntax implementation since the implementation does not
contain left recursion and is specified as LL(2) grammar. This section
is followed by the largest in this section and contains the language
semantic. It contains informal as well as formal parts. It reflects the
development of the language and the ideas behind the concepts. At last
a section deals with the concrete type rules and presents the current
...
more easy and clear. For a more detailed discussion see \Cref{ctr:discussion}
...
\begin{lstlisting}[float=h,language=ooplss,caption=Variable definition in local scope,label=lst:illegalNesting]
class A {
	var x : B;
...
\begin{lstlisting}[float=h,language=ooplss,caption=Subtyping in \ooplss,label=lst:subtypeOf]
class A {
...
\begin{lstlisting}[float=h,language=ooplss,caption=Subclassing in \ooplss,label=lst:subclassOf]
class A {
	var a : A;
...
\begin{lstlisting}[float=h,caption=Subclassing translated to Java,label=lst:javaTrans]
public class A {
	public A a;
...
\begin{lstlisting}[float=h,language=ooplss,caption=Illegal method m(o : \A) : \emph{Void} in class \B,label=lst:breakSelf]
class A {
	def m(o : A) : A {
...
\begin{lstlisting}[float=h,language=ooplss,caption=Type safe code with \mytype,label=lst:fixBreakSelf]
class A {
...
\begin{lstlisting}[float=h,caption=Subclassing with \mytype translated to Java,label=lst:javaTransMyTypeSubclassing]
public class A {
	public A m(A o) {
		m(this);
	}
...
\begin{lstlisting}[float=h,language=ooplss,caption=\mytype within subtyping contexts,label=lst:myTypeSubtyping]
class A {
	def m(o : MyType) : MyType {
		return m(self);
	}
...
\begin{lstlisting}[float=h,language=ooplss,caption=Mixed extension and inheritance hierarchy,label=lst:mixedHierarchy]
class A {
	def m(o : MyType) : MyType {
		return m(self);
	}
...
\begin{lstlisting}[float=h,language=ooplss,caption=Binding of \self and \mytype in a mixed context,label=lst:bindingInMixedHierarchy]
class A {
...
Knowing this fact generates the hierarchy in \Cref{fig:fixedHierarchy}
...
seems not very useful since even generics like those from Java do not
fit into this model very well. Furthermore, the matching relation is
not used by method parameters and is not yet introduced in \ooplss.
As seen in \Cref{ctr:makingSubclassingPractical} matching is used in object
...
from Java since Java only knows bounds on subtypes, so another more open,
...
From this thoughts, the C++ templates might be a good choice for
...
Even C++ templates have different semantics and are not equivalent with
the translation presented here for \ooplss, but the idea is similar
to create specially named classes similar to templates. Listing
...
of $T$ \match \A. The use of those classes is presented in Listing
\ref{lst:useParametrisationOoplss}. All of these assignments are typeable
and sound since all of these classes provide the methods assumed by the
...
\emph{Container}. The fact that an interface is needed will be presented
in the next section. One may remark that the Java interface of
...
signs as presented in Listing \ref{lst:useParametrisationJava}.

\begin{lstlisting}[float=h,language=ooplss,caption=Parametrisation in \ooplss,label=lst:parametrisationOoplss]
...
\begin{lstlisting}[float=h,language=ooplss,caption=Valid use of a match-bound container,label=lst:useParametrisationOoplss]
class Main {
	def run() : Void {
...
\begin{lstlisting}[float=h,language=java,caption=Container class translated to Java,label=lst:parametrisationJava]
...
\begin{lstlisting}[float=h,language=java,caption=Use of parametised classes in Java,label=lst:useParametrisationJava]
public class Main {
	public void run() {
...
now fits well in the model. However, since parametrised classes
...
\begin{lstlisting}[float=h,language=java,caption=Extending a container class translated to Java,label=lst:extendingContainer]
...
The idea of generating extended an class instead of using the
runtime generic support from Java is not new even though the idea
is a bit different with the same consequences. Robert Cartwright
and Guy Steele proposed another kind of generics in Java as a
concurrence to GJ\nomenclature{GJ}{The research language for the Java
generics. It is a short form of `Generic Java'. Due to copyright law,
the name is abbreviated} from Odersky and Wadler and is presented in
\cite{cartwright_compatible_1998}. Their project is called NextGen
and is based on Java 1.2. In contrast to GJ and the current generics
implementation in Java, this project provides an extension that is
not backward compatible to non generic code. However, since no generic
types are used at runtime, type parameters are not erased like in GJ
and Java 1.5.
...
parameters which Java has not.  Further the creation of new objects
would also be possible as long as a class provides a standard constructor.

As a drawback of creating all these classes a more sophisticated
bytecode analysis and engineering if a language would support a modular
code generation is needed, which \ooplss does not have so far.
...
and subclassing, is available, the type parameter list is extended in
every subtype with one additional type parameter Only this makes it
...
\begin{lstlisting}[float=h,language=ooplss,caption=Explicit \mytype,label=lst:explicitMyType]
class A[MyType <# A] {
	def m(o : MyType) MyType {

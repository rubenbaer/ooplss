...
\subsection{\mytype in Subtyping}
...
for the type \C. This fact makes it impossible to use a explicit recursive
...
\end{figure}
%
%\subsection{Method Overloading and Mutual Recursion}
%\subsection{}
...
The next listing \ref{lst:parametrisationOoplss} presents a container class
that is parametrised with a match-bound of \A. The use of thous classes
is presented in listing \ref{lst:useParametrisationOoplss}. All of these
assignments are typeable and sound since all these classes provides the
methods assumed by the bound. However, listing \ref{lst:parametrisationJava}
shows how this code is translated to Java and how the idea of templates is
adopted. Like in C++ the for every class a new inner class is created with
special names. To use this code in Java is straightforward translated in 
listing \ref{lst:useParametrisationJava}.
...
public abstract class Container<T> {
	...
	static public class Container$Foo$ extends Container<Foo> {
		public Foo m(t Foo) {
			t.someAction();
		}
	}

	static public class Container$Bar$ extends Container<Bar> {
		public Bar m(t Bar) {
			t.someAction();
		}
	}

	static public class Container$FooBar$ extends Container<FooBar> {
...
\begin{lstlisting}[float,language=java,caption=Extending a container class translated to Java,label=lst:useParametrisationJava]
public abstract class SpecialContainer<X> extends Container<X> {
	public abstract X f(X x);

	static public class SpecialContainer$Foo$ extends SpecialContainer<Foo> {
		public Foo m(Foo x) {
			return x;
		}
	}

	static public class SpecialContainer$Bar$ extends SpecialContainer<Bar> {
		public Bar m(Bar x) {
			return x;
		}
	}

	static public class SpecialContainer$FooBar$ extends SpecialContainer<FooBar> {

...
seems not very useful since even generics like those from Java don't
fit into this model very well. Furthermore, the matching relation is
not used by method parameters and does not appear yet in the language.
\todo{where exactly?}
...
Of course
\todo{Sounds like spoken... "of course"}
, C++ templates have different semantics and are not equivalent
with the translation presented here for \ooplss, but the idea is similar to
create special classes similar to templates.
\todo{Was si für special classes gmeint hie...?}

Listing \ref{lst:parametrisationOoplss} presents a container
class that is parametrised with a match-bound 
\todo{I think this is mentioned here for the first time... if not, reference?}
of \A. 
Class \B extends class \A and class \C inherits \A.
...
now fits well in the model. However, since parametrised classes can be too,
\todo{can be what?}
this is now shown that this is straightforward as well. \todo{reformulate}

For the
sake of simplicity only the translated Java code is presented because
fields that are not using parameters are translated in the same manner as 
in normal classes.
Listing \ref{lst:extendingContainer} shows an extended version
...
The idea of generating extended an class instead of using the runtime
generic support from Java is not new even though the idea is a bit different
with the same consequences. Robert Cartwright and Guy Steele proposed
another kind of generics in Java as a concurrence to GJ\todo{what is GJ?} from Odersky
and Wadler and is presented in \cite{cartwright_compatible_1998}. Their
project is called NextGen and is based \todo{Auf Java aufgebaut ist gemeint oder?}
on Java 1.2. However, since
no generic types are used at runtime, type parameters are not erased
like in GJ and Java 1.5.
...
parameters which Java has not. 
\todo{but java does have an instanceof operator!?}
Further the creation of new objects would
also be possible as long as a class provides a standard constructor.
...
To conclude
\todo{fyi: at last heisst ändlech}
this section about parametrisation let's consider the \mytype
construct. As seen in \Cref{ctr:theoreticalBackground}, the \self variable
can be bound to its real type by applying the unbound type
operator recursively. To grasp this notion in \ooplss, classes can be considered
as implicitly parametrised. Since both, subtyping and subclassing, are
available, the type parameter is extended in every subtype with one more
\todo{with one more what?}.
Only this makes it possible to derive binary methods in subtypes
and subclasses as well. An explicitly parametrised hypothetical use of

...
import ch.codedump.ooplss.antlr.OoplssGen2;
...
public class Main {
	...
	private void codeGeneration(Tree t, CommonTreeNodeStream nodes,
			SymbolTable symTab) throws IOException, RecognitionException,
			InterruptedException {
		...
		StringTemplateGroup templates = StringTemplateGroup.loadGroup("Ooplss");

		logger.finest(t.toStringTree());

		OoplssGen gen = new OoplssGen(nodes, symTab);
		gen.setTemplateLib(templates);
		OoplssGen.prog_return ret = gen.prog();

		// Get new generated class templates
	...
	/**
	 * Creates a temporary template group for the current output
	 * 
	 * @param template
	 *            Template string generated by OoplssGen
	 * @return
	 */
	private StringTemplateGroup CreateTemporaryTemplate(String template) {
		template = "group Output : Static;\n\n" + template;
		StringTemplateGroup superGroup = StringTemplateGroup
				.loadGroup("Static");
		logger.finest(template);
		StringTemplateGroup templateGroup = new StringTemplateGroup(
				new StringReader(template));
		templateGroup.setSuperGroup(superGroup);
	 ...
	 * 
	 * Decide what type of output was specified and based on that either write
	 * it to files or print it to stdout
	 * 
	 * @param string
	 * @throws IOException
	 * @throws InterruptedException
	...
	private Options defineOptions() {
		...
		options.addOption("n", "no-translation", false,
				"Don't translate to bytecode");
		options.addOption("k", "keep-java", false,
				"Keep the temporary java file");
